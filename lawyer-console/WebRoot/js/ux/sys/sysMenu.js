$package('otter.sysMenu');

otter.sysMenu = function(){
	var _box = null;
	var _this = {
		...
		addLine: function(data){
			...
			$(".remove-btn",line).click(function(){
				otter.confirm('Confirm','Are you sure you want to delete record?',function(r){
					if(r){
						_this.delLine(line);
					}
				})
  				...
  				title:'菜单列表',
	   			...
	   			columns:[[
					...
					{field:'childMenus',title:'子目录',width:120,align:'center',formatter:function(value,row,index){
						var html ="<a href='#' onclick='otter.sysMenu.toList("+row.id+")'>子菜单管理("+row.subCount+")</a>";
						return html;
					}}
				]],
				toolbar:[
