var YDataGrid = function(config){
		...
		var Handler = {
			...
			edit:  function(callback){
				...
				if (Utils.checkSelectOne(record)){
					...
					otter.getById(Action.getId,data,function(result){
						otter.closeProgress();
				...
				if (Utils.checkSelect(records)){
					$.messager.confirm('确认','确认删除记录?',function(r){  
					    if (r){
					    	...
					    	var data = arr.join("&");
					   		otter.deleteForm(Action.remove,data,function(result){
								otter.closeProgress();
								Events.refresh();
								//回调函数
								if(jQuery.isFunction(callback)){
			...
			save: function(callback){
				if(Form.edit.form('validate')){
					...
					otter.saveForm(Form.edit,function(data){
						otter.closeProgress();
		...
		var Utils = {
			...
			checkSelectOne : function(rows){//检查grid是否只勾选了一行,是返回 true,否返回true
				...
				otter.alert('警告','只能选择一行记录.','warning');  
				return false;
			}
		}
