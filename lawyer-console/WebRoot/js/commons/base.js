...
var otter={
	...
	checkLogin:function(data){//检查是否登录超时
		if(data.logoutFlag){
			otter.closeProgress();
			otter.alert('提示',"登录超时,点击确定重新登录.",'error',otter.toLogin);
			return false;
		}
		return true;
	},
			 	...
			 	error:function(response, textStatus, errorThrown){
			 		try{
				 		...
				 		if(!otter.checkLogin(data)){
					 		...
					 		otter.alert('提示', data.msg || "请求出现异常,请联系管理员 .",'error');
					 	}
			 		}catch(e){
			 			otter.alert('提示',"请求出现异常,请联系管理员 .",'error');
			 		}
			 	},
			 	complete:function(){
	...
	submitForm:function(form,callback,dataType){
			var option =
			{
			 ...
			 otter.ajaxSubmit(form,option);
	},
	saveForm:function(form,callback){
		if(form.form('validate')){
			otter.progress('Please waiting','Save ing...');
			//ajax提交form
